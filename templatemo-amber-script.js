...
class PhotoCoverflow {
   ...
   toggleAutoPlay() {
      ...
      if (this.isPlaying) {
    this.stopAutoPlay();
    playPauseBtn.innerHTML = '▶'; // Needs an exact match!
} else {
    this.startAutoPlay();
...
document.addEventListener('DOMContentLoaded', () => {
   ...
   // HIDE LOADING SCREEN AFTER THE MAGICAL POP IS COMPLETE
   setTimeout(() => {
      document.getElementById('loadingScreen').classList.add('hidden');
   }, 2400); // <-- CHANGED FROM 1000 TO 2400
...
// ==========================================
   ...
   if (themeSelect) {
      // Check if the user visited before and has a saved preference
      const savedTheme = localStorage.getItem('selected-portfolio-theme') || 'dark';
      
      // Initialize the theme immediately when page loads
      setActiveTheme(savedTheme);

      // Listen for when a user selects a different choice from the dropdown
   ...
   function setActiveTheme(themeName) {
      // Apply the theme attribute to the main HTML document tag
      document.documentElement.setAttribute('data-theme', themeName);
      
      // Remember selection for next visits
      localStorage.setItem('selected-portfolio-theme', themeName);
      
      // Ensure the dropdown shows the correct active option layout 
      if (themeSelect) {
